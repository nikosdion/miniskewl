 ...
 * --
 *
 * This version is modified with base URL handling by Nicholas K. Dionysopoulos
 *
 * @package 	cssmin
...
{
	if(empty(cssmin::$siteDomain))
	{
		cssmin::$siteDomain = rtrim(str_replace(JURI::base(true), '', JURI::base()),'/');
	}

	$baseurl = cssmin::$baseURL;
	if(!empty($baseurl)) {
		// Slash the protocol
	...
	if(
		...
		$myURL = rtrim(cssmin::$siteDomain.$baseurl,'/') .'/'. $myURL;
	}
